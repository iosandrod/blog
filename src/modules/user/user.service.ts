...
export class UserService {
	...
	async update(payload, params) {
		...
		await this.UserModel.update({ id }, upateInfoData);
		return '修改成功';
	}
	...
	async updateUserInfo(payload, params) {
		...
		const updateUserInfo = await this.UserModel.findOne({ id });
		...
		const { affected } = await this.UserModel.update({ id }, updateInfo);
		if (affected > 0) {
			return '修改成功';
		} else {
			throw new HttpException(
